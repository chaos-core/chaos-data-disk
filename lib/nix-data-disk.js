...
class ChaosDataDisk {
  ...
  constructor(chaos, config) {
    ...
    this._writer$
      .do((data) => this.chaos.logger.debug(`Write START: ${data.filename} => ${data.keyword} = ${data.value}`))
      .flatMap((data) => {
      ...
      .do((data) => this.chaos.logger.debug(`Write END: ${data.filename} => ${data.keyword} = ${data.value}`))
      .do((data) => data.callback(data.savedValue))
      .subscribe(() => this._writer$.request(1));
  }

  onListen() {
    this.chaos.shutdown$.subscribe(() => this._writeQueue$.onCompleted());
